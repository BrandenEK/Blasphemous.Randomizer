...
namespace Gameplay.UI.Others.UIGameLogic
{
	public class PlayerHealthPE02 : MonoBehaviour
	{
		...
		public static float StocksHeal
		{
			get
			{
				int num = PlayerHealthPE02.CurrentStocks + 1;
		...
		private void Update()
		{
			...
			this.prevFrameFillAmount = PlayerHealthPE02.FillAmount;
		}
		...
		{
			int maxStocks = PlayerHealthPE02.MaxStocks;
		...
		{
			if (this.prevFrameFillAmount == PlayerHealthPE02.FillAmount)
			{
				return;
			}
			this.prevFrameFillAmount = PlayerHealthPE02.FillAmount;
			if (PlayerHealthPE02.FillAmount >= 1f)
			{
			...
			else if (PlayerHealthPE02.FillAmount > 0f && PlayerHealthPE02.FillAmount < 1f)
			{
				...
				component2.fillAmount = PlayerHealthPE02.FillAmount;
			}
			else
			{
