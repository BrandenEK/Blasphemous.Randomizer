	...
	public class Spoiler
	{
		...
		private Dictionary<string, string> getNames()
		{
			...
			while (num != -1)
			{
				...
				dictionary.Add(text2.Substring(0, num3), text2.Substring(num3 + 2));
				num2 = num + 2;
		...
		private string GetRewardId(string id, Dictionary<string, Reward> rewards)
		{
			...
			default:
			{
				...
				if (progressiveReward.items[0] == "RB17")
				{
					return "RW";
				}
				if (progressiveReward.items[0] == "RB24")
				{
					return "BW";
				}
				if (progressiveReward.items[0] == "QI31")
				{
					return "TH";
				}
				return "x";
		...
		public string createSpoiler(Dictionary<string, Reward> rewards)
		{
			...
			string text = "";
			int num3;
			for (int num = 0; num != template.Length; num = num3)
			{
				...
				num3 = template.IndexOf('\r', num2 + 1);
				if (num3 < 0)
				{
					num3 = template.Length;
				}
				string rewardId = this.GetRewardId(template.Substring(num2 + 2, num3 - num2 - 2), rewards);
				string str = "x";
				if (names.ContainsKey(rewardId))
				{
					str = names[rewardId];
				}
				text = text + template.Substring(num, num2 + 2 - num) + str;
			}
			return text;
		}
	}
