...
namespace Framework.Randomizer
{
	public class Randomizer : GameSystem, PersistentInterface
	{
		...
		public override void Update()
		{
			...
			{
				string text = "Current Seed: " + this.seed;
				UIController.instance.ShowPopUp(text, "", 0f, false);
				return;
			}
			...
			{
				int type = 4;
				int id = this.collectCount + 1;
				this.collectCount = id;
				this.giveReward(new Reward(type, id));
			}
		}
		...
		private void Randomize(int seed)
		{
			...
			ForwardFiller forwardFiller = new ForwardFiller(this.gameConfig.randomizerSettings);
		...
		{
			new FileIO().write(message, "log.txt");
		}
		...
		{
			this.fileConfig = new FileIO().loadConfig();
			this.gameConfig = this.fileConfig;
