...
namespace Blasphemous.Randomizer.Zones;
...
public class ZoneHandler
{
    ...
    public void CollectLocation(string locationId, RandomizerSettings settings)
    {
        ItemLocation location = Main.Randomizer.DataHandler.ItemLocations[locationId];
        if (!ShouldTrackLocation(location, settings))
            return;
    ...
    public void ResetCollectionStatus(RandomizerSettings settings)
    {
        CollectionStatus = new Dictionary<string, ZoneCollection>();
        foreach (KeyValuePair<string, string> zoneEntry in Main.Randomizer.DataHandler.LocationNames)
        {
    ...
    private bool ShouldTrackLocation(ItemLocation location, RandomizerSettings settings)
    {
        if (!settings.ShuffleSwordSkills && location.Type == 1)
            return false;
        if (!settings.ShuffleThorns && location.Type == 2)
            return false;
        if (!settings.ShuffleBootsOfPleading && location.Id == "RE401")
            return false;
        if (!settings.ShufflePurifiedHand && location.Id == "RE402")
            return false;
        return true;
    }
